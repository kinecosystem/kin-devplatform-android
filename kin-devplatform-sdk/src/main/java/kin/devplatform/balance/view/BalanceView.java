		...
		this.arrow.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				if (arrow.getMeasuredWidth() > 0) {
					arrowFromX = arrow.getPivotX() - getResources()
						.getDimensionPixelOffset(R.dimen.kinecosystem_main_medium_margin);
	...
	public void animateArrow(boolean showArrow) {
		if (showArrow) {
			postOnAnimation(new Runnable() {
				@Override
				public void run() {
					balanceText.animate().translationX(arrowFromX).setInterpolator(new LinearInterpolator())
						.setDuration(ANIM_DURATION).start();
					arrow.animate().scaleX(FLOAT_1).translationX(arrowFromX).alpha(FLOAT_1)
						.setInterpolator(new LinearInterpolator()).setDuration(ANIM_DURATION).start();
				}
			});
		} else {
			postOnAnimation(new Runnable() {
				@Override
				public void run() {
					balanceText.animate().translationX(arrowToX).setInterpolator(new LinearInterpolator())
						.setDuration(ANIM_DURATION).start();
					arrow.animate().scaleX(FLOAT_08).translationX(arrowToX).alpha(FLOAT_0)
						.setInterpolator(new LinearInterpolator()).setDuration(ANIM_DURATION).start();
				}
			});
		}

...
public final class KinEcosystemInitiator {
	 ...
	 */
	public void internalInit(Context context) { // TODO: 13/01/2019 need to test and see if everything is work when we simulate process restart.
	...
	private void init(Context context, String appId, KinEnvironment environment, boolean withLogin,
		...
		if (!isInitialized) {
			...
			if (appId == null && BlockchainSourceImpl.getInstance() != null) {
				appId = BlockchainSourceImpl.getInstance().getAppId(); // TODO: 13/01/2019 need to see if is is working in process restart
			}
	...
	private void handleMigration(final Context context, final String appId, final EventLogger eventLogger, final MigrationManager migrationManager,
								 ...
								 final KinCallback<Void> loginCallback, final KinMigrationListener migrationCallback) throws MigrationInProcessException {
